...
# Persistent JSON file-based cache to survive app restarts and minimize AIPipe token usage
...
async def chat(messages, model=None, max_tokens=800, force_json=True, retries=4):
    key = _ck("chat", model or config.TEXT_MODEL, json.dumps(messages, sort_keys=True, default=str))
    if key in _CACHE:
        print(f"Cache hit for chat (model: {model or config.TEXT_MODEL})")
        return _CACHE[key]
    ...
    async with httpx.AsyncClient(timeout=90) as c:
        for attempt in range(retries):
            current_head = {"Authorization": f"Bearer {config.AIPIPE_TOKEN}", "Content-Type": "application/json"}
            r = await c.post(f"{config.AIPIPE_BASE}/chat/completions",
                             headers=current_head, json=body)
...
async def embed(input_texts, model=None):
    key = _ck("embed", model or config.EMBED_MODEL, json.dumps(input_texts, sort_keys=True))
    if key in _CACHE:
        print(f"Cache hit for embeddings (model: {model or config.EMBED_MODEL})")
        return _CACHE[key]
    
    async with httpx.AsyncClient(timeout=90) as c:
        current_head = {"Authorization": f"Bearer {config.AIPIPE_TOKEN}", "Content-Type": "application/json"}
        r = await c.post(f"{config.AIPIPE_BASE}/embeddings", headers=current_head,
                         json={"model": model or config.EMBED_MODEL,
                               "input": input_texts})
        r.raise_for_status()
...
async def gemini_transcribe(payload, attempts_per_model=3):
    ...
    # Cache based on the audio data itself to avoid repeated transcriber calls
    try:
        audio_b64 = payload["contents"][0]["parts"][1]["inlineData"]["data"]
        key = _ck("transcribe", hashlib.sha256(audio_b64.encode()).hexdigest())
        if key in _CACHE:
            print("Cache hit for audio transcription")
            return _CACHE[key]
    except Exception:
        key = None
